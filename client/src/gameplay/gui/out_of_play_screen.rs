            ...
            )
        {
            ...
            egui::collapsing_header::CollapsingState
                ...
                .show_header(ui, |ui| {
                    ...
                    ui.label(format!("--- Cost: {}c --- ", inventory.cached_current_price));
                    ui.add(
                        egui::ProgressBar
        ...
        ui.horizontal(|ui| {
            ...
            if module_can_buy_from_player {
                if players_current_amount > 0 {
                    ...
                    if players_current_amount > 5 {
                        sell_item_to_station(ctx, 5, docked_ship, module, inventory, ui);
                    }
                    if players_current_amount > 10 {
                        sell_item_to_station(ctx, 5, docked_ship, module, inventory, ui);
                    }
        ...
        ui.horizontal(|ui| {
            ...
            if module_can_sell_to_player {
                if inventory.quantity > 0 {
                    buy_item_from_station(ctx, 1, docked_ship, module, inventory, ui);
                    if inventory.quantity > 2 {
                        buy_item_from_station(ctx, 2, docked_ship, module, inventory, ui);
                    }
                    if inventory.quantity > 3 {
                        buy_item_from_station(ctx, 3, docked_ship, module, inventory, ui);
                    }
                    ...
                    if inventory.quantity > 25 {
                        buy_item_from_station(ctx, 25, docked_ship, module, inventory, ui);
                    }
                    if inventory.quantity > 100 {
                        buy_item_from_station(ctx, 100, docked_ship, module, inventory, ui);
                    }
                    if inventory.quantity > 6 {
                        buy_item_from_station(
                            ctx,
                            (inventory.quantity as u32) / 2,
                            docked_ship,
                            module,
                            inventory,
                            ui
