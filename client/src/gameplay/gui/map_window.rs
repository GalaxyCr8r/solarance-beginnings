  ...
  fn draw_galaxy_map(&self, ui: &mut egui::Ui, ctx: &DbConnection) {
    ...
    Frame::canvas(ui.style()).show(ui, |ui| {
      ...
      for (i, object) in ctx.db.star_system_object().iter().enumerate() {
        ...
        let stroke = match object.kind {
            StarSystemObjectKind::Star => {
              Stroke { width: 3., color: Color32::YELLOW }
            },
            StarSystemObjectKind::Planet => Stroke { width: 1., color: Color32::LIGHT_BLUE },
            StarSystemObjectKind::Moon => Stroke { width: 1., color: Color32::GRAY },
            StarSystemObjectKind::AsteroidBelt => {
              Stroke { width: object.rotation_or_width_km, color: Color32::from_rgba_unmultiplied(115, 52, 32, 16) }
            },
            StarSystemObjectKind::NebulaBelt => {
              Stroke { width: object.rotation_or_width_km, color: Color32::from_rgba_unmultiplied(181, 69, 255, 16) }
            },
        };

        match object.kind {
          StarSystemObjectKind::Star | StarSystemObjectKind::Planet | StarSystemObjectKind::Moon =>
          {
            let point_in_system = glam::Vec2::from_angle(object.rotation_or_width_km) * object.orbit_au;

            let mut point = pos2(point_in_system.x * zoom_px + offset.x, point_in_system.y * zoom_px + offset.y);        
          ...
          StarSystemObjectKind::AsteroidBelt | StarSystemObjectKind::NebulaBelt =>
          {
            ...
            shapes.push(Shape::circle_stroke(point_in_screen,
              object.orbit_au, stroke
              ));
          }
        }
      }
      ...
      for (i, sector) in ctx.db.sector().iter().enumerate() {
        ...
        let min = Pos2::new(point_in_screen.x - sector_radius, point_in_screen.y - sector_radius);
        let max = Pos2::new(point_in_screen.x + sector_radius, point_in_screen.y + sector_radius);
        shapes.push(Shape::rect_stroke(
          egui::Rect { min, max },
          CornerRadius {nw: 0, ne: 4, sw: 4, se: 4 }, 
          stroke, StrokeKind::Middle));
      }
