...
pub fn is_server_or_sobj_owner(ctx: &ReducerContext, stellar_object_id: Option<StellarObjectId>) -> Result<(), String> {
    let dsl = dsl(ctx);
...
pub fn is_server_or_ship_owner(ctx: &ReducerContext, ship_id: Option<ShipGlobalId>) -> Result<(), String> {
    let dsl = dsl(ctx);
    ...
    if let Some(s_id) = ship_id {
        let ship = dsl.get_ship_by_id(s_id).ok_or(IS_SERVER_OR_OWNER_ERROR)?;
