...
pub fn create_mining_timer_for_ship(
    ...
    dsl.get_ship_mining_timers_by_ship_sobj_id(ship_sobj_id)
        .for_each(|timer| {
...
pub fn ship_status_timer_reducer(
    ...
    // Get ship rows
    let ship_type = dsl
        .get_ship_type_definition_by_id(timer.get_ship_type_id())
        .ok_or("Oh noe couldn't find ship type!")?;
    let mut ship_status = dsl
        .get_ship_status_by_id(timer.get_ship_id())
...
pub fn ship_mining_timer_reducer(
    ...
    let ship_object = dsl
        .get_ship_by_sobj_id(timer.get_ship_sobj_id())
        .ok_or(format!(
            "Failed to find ship object for mining timer: {:?} Removed timer.",
            timer.get_ship_sobj_id()
        ))?;
    let mut asteroid_object = dsl
        .get_asteroid_by_sobj_id(timer.get_asteroid_sobj_id())
        .ok_or(format!(
            "Failed to find asteroid object for mining timer: {:?} Removed timer.",
            timer.get_ship_sobj_id()
        ))?;
    ...
    let mut ship_status = dsl
        .get_ship_status_by_id(ship_object.get_id())
        .ok_or_else(|| {
            dsl.delete_ship_mining_timer_by_scheduled_id(timer.get_scheduled_id());
            format!(
...
pub fn ship_add_cargo_timer_reducer(
    ...
    if let Some(ship_object) = dsl.get_ship_by_id(timer.get_ship_id()) {
        let mut ship_status = dsl
            .get_ship_status_by_id(timer.get_ship_id())
            .ok_or(format!(
                "Failed to get ship status row for ship #{}",
                timer.ship_id
            ))?;
