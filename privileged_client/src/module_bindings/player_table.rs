...
/// Access to the `username` unique index on the table `player`,
/// which allows point queries on the field of the same name
/// via the [`PlayerUsernameUnique::find`] method.
///
/// Users are encouraged not to explicitly reference this type,
/// but to directly chain method calls,
/// like `ctx.db.player().username().find(...)`.
pub struct PlayerUsernameUnique<'ctx> {
    imp: __sdk::UniqueConstraintHandle<Player, String>,
    phantom: std::marker::PhantomData<&'ctx super::RemoteTables>,
}

impl<'ctx> PlayerTableHandle<'ctx> {
    /// Get a handle on the `username` unique index on the table `player`.
    pub fn username(&self) -> PlayerUsernameUnique<'ctx> {
...
impl<'ctx> PlayerUsernameUnique<'ctx> {
